    ...
    let sock = Object.defineProperties(conn, {
        ...
        getJid: {
            value(sender) {
                ...
                // Search contacts first (covers private DMs)
                if (conn.contacts) {
                    for (let [jid, contact] of Object.entries(conn.contacts)) {
                        if (contact?.lid === sender && jid.endsWith('@s.whatsapp.net')) {
                            return conn.isLid[sender] = jid;
                        }
                    }
                }

                // Fall back to group participant lists
...
async value (status) {
        ...
        // <iq to="s.whatsapp.net" type="set" xmlns="status" id="21168.6213-69"><status>"Hi, A IM  NovaChat "</status></iq>
    }
},
        sendStickerFromUrl: {
...
let pathFile1 = await writeExif(media, { packname: options.packname ? options.packname : 'NovaChat By SeeMoo', author: options.author ? options.author : '', categories: options.categories ? options.categories : [] })
